...
function Quote({ typingContents }) {
  //Data preprocessing
  let allTypingContents = typingContents.reduce((acc, curr) => {
  ...
  const [quote, setQuote] = useState(
    allTypingContents[Math.floor(Math.random() * allTypingContents.length) + 1]
  )
  ...
  let renderNextQuote = () => {
    setQuote(allTypingContents[Math.floor(Math.random() * allTypingContents.length) + 1])
  }
