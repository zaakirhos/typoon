...
function Quote({ quotes }) {
  ...
  let handleChange = e => {
    ...
    let unTypedLength = e.target.value? quoteLetters.length - e.target.value.length : 0;

    for (let index = unTypedLength; index < quote.length; index++) {
  ...
  useEffect(() => {
    for (let index = 0; index < quoteRef.current.children.length; index++) {
      quoteRef.current.children[index].style.color = "black";
