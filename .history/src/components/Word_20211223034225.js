...
function Word({ words }) {
  ...
  let renderNextQuote = () => {
    setWord(allWords[Math.floor(Math.random() * allWords.length) + 1]).replace(
      /./,
      c => c.toUpperCase()
    )
  }
  ...
  return (
    <div>
      ...
      <WordContainer>
        ...
        <br />
        <WordInput autofocus onChange={handleChange}></WordInput>
      </WordContainer>
      <Stat typingContent={word} type='word' incorrect={incorrectLetters}></Stat>
    </div>
  )
}
