...
function Quote({quotes}) {
  ...
  let handleChange = (e) => {
    ...
    let renderQuoteChar = (char, index) => {
      return (
        <span key={index} className="quote-char">{char}</span>
        )
    }
  ...
  return (
    <div>
      ...
      <div class="container">
        <div class="quote-display" id="quoteDisplay">{quote.text.split("").map(renderQuoteChar)}</div>
        <i style={{ color: "gray" }}>
          &mdash;{" "}
