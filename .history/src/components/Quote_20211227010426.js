...
if(type === "word"){
  ...
  const quoteLetters = type=== "quote"? typingText.text.split("") : typingText.split("")
  ...
  let handleChange = e => {
    ...
    if (e.target.value === typingText.text) {
      renderNextQuote()
  ...
  let renderNextQuote = () => {
    setQuote(flattenedArray[Math.floor(Math.random() * flattenedArray.length) + 1])
  }
