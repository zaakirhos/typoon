...
function Quote({ textArray, type }) {
  ...
  let flattenedArray = [];
  if(type === "quote"){
  let flattenedArray = textArray.reduce((acc, curr) => {
  ...
  const [quote, setQuote] = useState(
    flatlet flattenedArray[Math.floor(Math.random() * flatlet flattenedArray.length) + 1]
  )
  ...
  let renderNextQuote = () => {
    setQuote(flatlet flattenedArray[Math.floor(Math.random() * flatlet flattenedArray.length) + 1])
  }
