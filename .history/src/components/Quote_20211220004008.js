...
const QuoteChar = styled.span`
  font-size: 1.5em;
  color: ${props => props.isTyped ? (props.isCorrect ? "green" : "red") : "black"};
`
...
function Quote({ quotes }) {
  ...
  const [quoteCharColor, setQuoteCharColor] = useState("black")
  ...
  let handleChange = e => {
    ...
    e.target.value.split("").forEach((char, i) => {
      if (char === quoteLetters[i]) {
        setQuoteCharColor("green")
      }
      if (char !== quoteLetters[i]) {
        setQuoteCharColor("red")
      }
    })
    }
  ...
  let renderQuoteChar = (char, index) => {
    return <QuoteChar key={index} color={quoteCharColor}>{char}</QuoteChar>
  }
